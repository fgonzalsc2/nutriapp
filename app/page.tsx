...
export default function Home() {
  ...
  const [mensajeAlerta, setMensajeAlerta] = useState("");
    ...
    const generarMenuIA = async () => {
        ...
        if (!process.env.NEXT_PUBLIC_GEMINI_API_KEY) return alert("Falta la API Key en .env.local");
        ...
        try {
            ...
            const prompt = `
                Actúa como un Chef Nutricionista experto. Crea un ejemplo de MENÚ DE UN DÍA (Desayuno, Almuerzo, Cena)
                respetando EXACTAMENTE estas porciones. Sé creativo y apetitoso.
                
                - Desayuno: ${grid.desayuno.cereales} cereales, ${grid.desayuno.proteina} proteínas, ${grid.desayuno.lacteos} lácteos, ${grid.desayuno.frutas} fruta.
                - Almuerzo: ${grid.almuerzo.cereales} cereales, ${grid.almuerzo.proteina} proteínas, ${grid.almuerzo.verduras} verduras, ${grid.almuerzo.grasas} grasas.
                - Cena: ${grid.cena.cereales} cereales, ${grid.cena.proteina} proteínas, ${grid.cena.verduras} verduras.

                Formato de respuesta:
                🍳 **Desayuno:** [Nombre del plato] (Ingredientes breves)
                🍲 **Almuerzo:** [Nombre del plato] (Ingredientes breves)
                🌙 **Cena:** [Nombre del plato] (Ingredientes breves)
                
                No des introducciones, solo el menú.
            `;

            const result = await model.generateContent(prompt);
            const response = await result.response;
            setMenuIA(response.text());
        } catch (error) {
            console.error("Error IA:", error);
            alert("Error al conectar con el Chef IA. Revisa tu API Key.");
        }
        setCargandoIA(false);
    };
