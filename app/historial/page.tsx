...
export default function HistorialPage() {
    ...
    const procesarDashboard = (data: any[]) => {
        ...
        setStats({ pacientesMes, inactivos: listaRiesgo.length, topObjetivo: topObj === 'bajar' ? 'Pérdida Peso' : topObj });
    ...
    const analizarPlatoIA = async () => {
        ...
        try {
            ...
            reader.onloadend = async () => {
                ...
                const prompt = "Actúa como un Nutricionista Experto. Analiza esta foto de comida. 1. Identifica los alimentos principales. 2. Estima las calorías totales (aproximadas). 3. Da un veredicto nutricional breve (1 frase): ¿Es equilibrado? ¿Falta proteína? ¿Exceso de carbos? Sé directo y conciso.";
            ...
            // Esto mostrará el error real en la pantalla en lugar de quedarse cargando
            setResultadoIA(`⚠️ Error: ${error.message || "No se pudo conectar con Gemini 2.5"}`);
    ...
    const enviarWsp = (nombre: string) => window.open(`https://wa.me/?text=${encodeURIComponent(`Hola ${nombre}, hace tiempo no te veo. ¿Cómo vas con la pauta?`)}`, '_blank');
    ...
    const getObjetivoBadge = (o:string)=> o==='bajar'?<span className="text-red-600 font-bold text-[10px] bg-red-50 px-2 py-1 rounded">DÉFICIT</span>:<span className="text-blue-600 font-bold text-[10px] bg-blue-50 px-2 py-1 rounded">MANTENCIÓN</span>;
            ...
            <div className="max-w-6xl mx-auto space-y-6">
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 relative overflow-hidden group">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-end mt-2 gap-4">
                        <div>
                            ...
                            <p className="text-xs text-slate-500 mt-1 font-medium">{finanzas.faltante > 0 ? `🔥 Faltan ${Math.ceil(finanzas.faltante / config.valor)} consultas.` : "🎉 ¡Meta superada!"}</p>
                        </div>
                        <div className="flex gap-2">
                             {/* BOTÓN MAGICO DE IA AQUI */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className={`p-5 rounded-2xl shadow-sm border flex items-center gap-4 transition relative overflow-hidden ${stats.inactivos > 0 ? 'bg-white border-red-200' : 'bg-white border-slate-100'}`}>
                        {stats.inactivos > 0 && <div className="absolute right-0 top-0 bg-red-500 text-white text-[9px] font-bold px-2 py-0.5 rounded-bl-lg">ACCIÓN</div>}
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                        <h3 className="font-bold text-slate-700 flex items-center gap-2"><FileText className="w-4 h-4 text-slate-400" /> Historial Clínico</h3>
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-slate-50 border-b border-slate-200 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                            <tr><th className="p-4">Paciente</th><th className="p-4">Fecha</th><th className="p-4 text-center">Calorías</th><th className="p-4 text-center">Objetivo</th><th className="p-4 text-right">Acciones</th></tr>
                        </thead>
